	...
	for {
		if renewableOrder != nil {
			...
			if order.Status == "CANCELED" || order.Status == "EXPIRED" {
				renewableOrder = nil
				log.Println("Убран ордер из списка наблюдения с направлением",
...
func createLinkStopLossOrder(buyOrder *binance.Order, stopLossOrder *binance.Order, client *bnc.API) {
	for {
		if buyOrder != nil && buyOrder.Status != "EXPIRED" && stopLossOrder == nil {
			if buyOrder.Status == "FILLED" {
				...
				log.Println("Добавлен STOP-LOSS ордер с направлением", stopLossOrder.Symbol,

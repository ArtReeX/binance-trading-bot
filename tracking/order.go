...
func trackStopLossOrder(pair string, id int64, status *BotStatus, client *bnc.API) {
	for {
		order, err := client.GetOrder(pair, id)
		if err != nil {
			log.Println(err)
		...
		if OrderStatus(order.Status) == OrderStatusCanceled && *status != BotStatusActiveSell {
			...
			*status = BotStatusWaitPurchase
			...
			// если продажа была выполнена перестаём отслеживание
			return
		}
	}
}
